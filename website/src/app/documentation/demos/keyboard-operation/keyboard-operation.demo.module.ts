/*
 * Copyright (c) 2018-2026 Porsche Informatik. All Rights Reserved.
 * This software is released under MIT license.
 * The full license information can be found in LICENSE in the root directory of this project.
 */
import { NgModule } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { RouterModule } from '@angular/router';
import { ClarityModule, ClrFormsModule } from '@clr/angular';
import {
  ClrAddonsModule,
  CLR_CONTROL_ENTER_SUBMIT_TRANSLATION,
  ClrControlEnterSubmitTranslationService,
} from '@porscheinformatik/clr-addons';
import { UtilsModule } from '../../../utils/utils.module';
import { DocWrapperModule } from '../_doc-wrapper/doc-wrapper.module';

import { KeyboardOperationDemo } from './keyboard-operation.demo';

class ControlEnterTranslationService implements ClrControlEnterSubmitTranslationService {
  translate(key: string): string {
    if (key === 'SUBMIT_KEY') {
      return 'Ctrl+Enter to submit';
    }
    return '';
  }
}

@NgModule({
  imports: [
    CommonModule,
    FormsModule,
    ClarityModule,
    ClrFormsModule,
    ClrAddonsModule,
    UtilsModule,
    DocWrapperModule,
    RouterModule.forChild([{ path: '', component: KeyboardOperationDemo }]),
  ],
  declarations: [KeyboardOperationDemo],
  exports: [KeyboardOperationDemo],
  providers: [
    {
      provide: CLR_CONTROL_ENTER_SUBMIT_TRANSLATION,
      useClass: ControlEnterTranslationService,
    },
  ],
})
export class KeyboardOperationDemoModule {}
