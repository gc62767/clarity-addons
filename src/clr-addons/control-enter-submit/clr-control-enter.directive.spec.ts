...
} from './clr-control-enter-submit.directive';
...
@Component({
  template: `
    <form ngForm [clrControlEnterSubmit]="'TEST_KEY'">
      <input name="inputField" />
      <button type="submit" class="submit-btn">Submit</button>
    </form>
  `,
...
class TestComponentWithEmptyKey {}
...
describe('ControlEnterDirective Tooltip Behavior', () => {
  ...
  let translationServiceMock: jasmine.SpyObj<ClrControlEnterSubmitTranslationService>;

  beforeEach(async () => {
    translationServiceMock = jasmine.createSpyObj('ClrControlEnterSubmitTranslationService', ['translate']);
    // Default mock behavior
    translationServiceMock.translate.and.returnValue('Translated Tooltip');
  });

  describe('With Translation Service Provided', () => {
    beforeEach(async () => {
      await TestBed.configureTestingModule({
        declarations: [ClrControlEnterSubmitDirective, TestComponentWithKey, TestComponentWithEmptyKey],
        imports: [ReactiveFormsModule],
        providers: [
          {
            provide: ClrForm,
    ...
    it('should set title attribute when key is provided and translation exists', () => {
      fixture = TestBed.createComponent(TestComponentWithKey);
      fixture.detectChanges();
      const submitButton = fixture.nativeElement.querySelector('button[type="submit"]');
      expect(submitButton.getAttribute('title')).toBe('Translated Tooltip');
      expect(translationServiceMock.translate).toHaveBeenCalledWith('TEST_KEY');
    });

    it('should NOT set title attribute when translation returns empty string', () => {
    ...
    it('should NOT set title attribute when key is explicitly empty', () => {
      fixture = TestBed.createComponent(TestComponentWithEmptyKey);
      fixture.detectChanges();
      const submitButton = fixture.nativeElement.querySelector('button[type="submit"]');
      expect(submitButton.getAttribute('title')).toBeNull();
  ...
  describe('Without Translation Service Provided', () => {
    beforeEach(async () => {
      await TestBed.configureTestingModule({
        declarations: [ClrControlEnterSubmitDirective, TestComponentWithKey],
        imports: [ReactiveFormsModule],
        providers: [
          {
            provide: ClrForm,
          ...
          // No CLR_CONTROL_ENTER_SUBMIT_TRANSLATION provided
        ],
      }).compileComponents();
    });

    it('should NOT set title attribute and fail silently when key provided but no service', () => {
      expect(() => {
        fixture = TestBed.createComponent(TestComponentWithKey);
        fixture.detectChanges();
      }).not.toThrow();
